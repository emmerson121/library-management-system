...
type UserRole = "Student" | "Author" | "Library Attendant";
...
const ROLES: UserRole[] = ["Student", "Author", "Library Attendant"];

const registeredUsers: Record<string, { name: string; password: string; role: UserRole }> = {};
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      ...
      if (mode === "signup") {
        ...
        registeredUsers[email] = { name: title.trim(), password, role };
        onSuccess({ name: title.trim(), email, role, loans: [] });
      } else {
        ...
        onSuccess({ name: stored.name, email, role: stored.role, loans: [] });
      }
    }, 700);
  };
          ...
          <div style={{ display: "flex", flexDirection: "column" as const, gap: "14px" }}>
            {mode === "signup" && (
              <label style={labelStyle}>
                ...
                <input type="text" placeholder="e.g. Ada Lovelace" value={title}
